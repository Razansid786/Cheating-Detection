...
model_path1 = "models_cache/models/YOLOv8n.pt"
model_path2 = "models_cache/models/YOLOv8m_best.pt"

person_model = YOLO(model_path1)
behavior_model = YOLO(model_path2)
...
#     download_path = os.join(OUTPUT_FOLDER,f"{os.path.splitext(filename)[0]}_out.mp4")
...
def show_results(filename):
    ...
    # FIXED: os.path.join instead of os.join
    ...
    return render_template('results.html',
                           ...
                           summary=json.dumps(summary),
                           output_video=output_video_path,
                           download_path=download_path) # Passing the specific path you requested

# NEW: Route to handle the download using the exact path logic you provided
@app.route('/download_result/<filename>')
def download_result(filename):
    # Strictly using the download_path logic you defined
    download_path = os.path.join(OUTPUT_FOLDER, f"{os.path.splitext(filename)[0]}_out.mp4")
...
def delete_video(filename):
    # filepath  = os.path.join(UPLOAD_FOLDER, filename)
    # filepath1 = os.path.join(OUTPUT_FOLDER, filename + '_out.mp4')
    # filepath2 = os.path.join(SUMMARY_FOLDER, filename + '-summary.json')
    # filepath3 = os.path.join("static", "outputs", filename + '_out.mp4')

    # deleted = False

    # for path in [filepath, filepath1, filepath2, filepath3]:
    ...
    # if deleted:
    #     return jsonify({'success': True, 'message': 'File(s) deleted'}), 200
    # else:
    #     return jsonify({'error': 'File not found'}), 404
    # Extract name without extension and extension separately
    ...
    filepaths = [
        # Original uploaded file (with extension)
        os.path.join(UPLOAD_FOLDER, filename),
        
        # Output video - remove original extension and add _out + same extension
        os.path.join(OUTPUT_FOLDER, name_without_ext + '_out' + ext),
        
        # Summary file - remove original extension and add -summary + .json
        os.path.join(SUMMARY_FOLDER, name_without_ext + '_summary.json'),
        
        # Static outputs - same as output video
